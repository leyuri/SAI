...
- Mapbox API + 공공데이터 포탈 API + kakao 좌표 변환 API
...
기존에 있는 서비스의 경우 좌표상의 중간 지점을 알려주기 때문에 실질적으로 만남이 불가능한 장소(산 또는 강 한 가운데)를 추천해주는 경우가 많음. 따라서 실질적인 만남이 이루어질 수 있는 장소와 인근 힙플레이스 정보를 제공받을 수 있다면 편리할 것으로 전망함.
...
중간지점 분석 알고리즘: 실제 소요시간(대중교통 이용)을 고려하여 여러명에게 최적의 중간지점을 분석하여 추천 <br/>
힙플레이스 추천: 빅데이터를 활용하여 저장되어 있는 최적의 장소 추천 시스템을 제공 <br/>
경로: 최적의 중간 지점까지 가는 경로를 최소 환승, 최단 경로로 표시해서 사용자에게 편의성 제공 <br/>
코스 추천: 중간 지점 분석 알고리즘에 참여한 사용자들의 선호도를 기반으로 코스를 추천하여 활용성 향상 <br/>
반응형 웹: 어떠한 디바이스에서 서비스를 요청하더라도 화면 크기에 알맞게 서비스를 제공 <br/>
